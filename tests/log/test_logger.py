...
class TestLogger(unittest.TestCase):
    def setUp(self):
        ...
        config_root_logger(level=logging.DEBUG)
        # get a logger for this session
        self.log = logging.getLogger(__name__)
        # set the paths for the logs (comes from reading logging_config.json)
        self.error_path = '../../opendeep/log/logs/error/errors.log'
        self.info_path  = '../../opendeep/log/logs/info/info.log'
    ...
    def testDebugConsole(self):
        ...
        self.assertIn('Unit testing debug.', output)
    ...
    def testInfoFile(self):
        ...
        self.assertIn('Unit testing info.', text[-2])
        self.assertIn('Unit testing warning.', text[-1])
    ...
    def testErrorFile(self):
        ...
        with open(self.error_path, 'r') as f:
            text = f.readlines()
        # should be last lines in file
        self.assertIn('Unit testing error.', text[-2])
        self.assertIn('Unit testing critical.', text[-1])
