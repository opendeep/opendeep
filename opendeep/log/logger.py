...
def config_root_logger(config_file='logging_config.json', level=logging.DEBUG):
    """
    ...
    level : str, optional
        The minimum importance level to start logging.
    """
    ...
    if os.path.exists(path):
        with open(path, 'rt') as f:
            try:
                ...
                logger.exception('Exception in reading the JSON logging config file! {!s}'.format(e.message))
        ...
        if init:
            ...
            if init:
                try:
                    ...
                    logger = get_root_logger()
                    logger.exception(
                        'Exception in loading the JSON logging config file to the logging module! {!s}'.format(
                            e.message
                        )
                    )
                    logger.warning('Anyway, loading the basicConfig for the logger instead.')
    ...
    # otherwise, couldn't find the configuration file
        ...
        logging.basicConfig(level=level)
        logger = get_root_logger()
        logger.warning("Could not find configuration file for logger! Was looking for {!s}. "
