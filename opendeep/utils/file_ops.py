...
def mkdir_p(path):
    ...
    log.debug('Making directory {!s}'.format(path))
    try:
        os.makedirs(path)
    except OSError as e:  # Python >2.5
        if e.errno == errno.EEXIST and os.path.isdir(path):
            log.debug('Directory {!s} already exists!'.format(path))
            pass
        else:
            log.exception('Error making directory {!s}'.format(path))
            raise
            ...
            for root, dirs, files in os.walk(path):
                for basename in files:
                    ...
                    try:
                        ...
                        log.exception('TypeError exception when finding files. {!s}'.format(te.message))
                        raise
...
def download_file(url, destination):
    ...
    log.debug('Downloading data from {!s} to {!s}'.format(url, destination))
    try:
        page = urlopen(url)
        if page.getcode() is not 200:
            log.warning('Tried to download data from {!s} and got http response code {!s}'.format(url, page.getcode()))
            return False
        urlretrieve(url, destination)
        return True
    except Exception as e:
        log.exception('Error downloading data from {!s} to {!s} : {!s}'.format(url, destination, e.message))
        return False
...
def get_file_type(file_path):
    ...
    if os.path.exists(file_path):
        ...
        # otherwise if it is a file
            ...
            log.warning("File {!s} isn't a file or directory, but it exists... WHAT ARE YOU?!?".format(file_path))
            return UNKNOWN
    else:
        log.debug("File {!s} doesn't exist!".format(file_path))
        return None
...
def get_extension_type(filename):
    ...
    if extension == '.zip':
        ...
        log.warning("Didn't recognize file extension {!s} for file {!s}".format(extension, filename))
        return UNKNOWN
...
def unzip(source_filename, destination_dir='.'):
    ...
    log.debug('Unzipping data from {!s} to {!s}'.format(source_filename, destination_dir))
    try:
        ...
        log.exception(
            'Error unzipping data from {!s} to {!s} : {!s}'.format(source_filename, destination_dir, e.message)
        )
        return False
...
def untar(source_filename, destination_dir='.'):
    ...
    source_filename = os.path.realpath(source_filename)
    destination_dir = os.path.realpath(destination_dir)
    log.debug('Unzipping tarball data from {!s} to {!s}'.format(source_filename, destination_dir))
    try:
        ...
        log.exception(
            'Error unzipping tarball data from {!s} to {!s} : {!s}'.format(source_filename, destination_dir, e.message)
        )
        return False
...
def gunzip(source_filename, destination_dir='.'):
    ...
    source_filename = os.path.realpath(source_filename)
    destination_dir = os.path.realpath(destination_dir)
    log.debug('Unzipping gz data from {!s} to {!s}'.format(source_filename, destination_dir))
    ...
    try:
        ...
        log.exception(
            'Error unzipping gz data from {!s} to {!s} : {!s}'.format(source_filename, destination_dir, e.message)
        )
        return False
...
def install(path, source=None):
    ...
    found = False
    log.info('Installing dataset {!s}'.format(path))
    ...
    try:
        ...
        log.error(
            "Couldn't make the dataset path with directory {!s} and path {!s}".format(dataset_dir, path)
        )
    ...
    if not is_dir:
        ...
        while not found and ext_idx <= len(split_fname):
            if os.path.exists(os.path.join(dirs, accumulated_name)):
                ...
                log.debug('Found file {!s}'.format(path))
    ...
    if not found:
        if source is not None and file_type is None:
            ...
            msg = "Filename {!s} couldn't be found, and no URL source to download was provided.".format(path)
            log.error(msg)
    ...
    if file_type is not get_file_type(path) and download_dest is not None:
        ...
        if unzip_success:
            ...
            log.debug('Removing file {!s}'.format(download_dest))
