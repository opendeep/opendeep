...
def get_shared_values(variables, borrow=False):
    ...
    try:
        ...
        log.exception("Cannot get values, there was an AttributeError {!s}".format(e.message))
        raise
    return values
...
def set_shared_values(variables, values, borrow=False):
    ...
    for variable, value in safe_zip(variables, values):
        ...
        assert value.shape == variable.get_value().shape, \
            "Shape mismatch! Value had shape {!s}, expected {!s}".format(value.shape, variable.get_value().shape)
        try:
            variable.set_value(value, borrow=borrow)
        except AttributeError as e:
            log.exception("Cannot set values, there was an AttributeError {!s}".format(e.message))
            raise
...
def numpy_one_hot(vector, n_classes=None):
    ...
    assert vector.ndim == 1, "Dimension mismatch for input vector, found {!s} dimensions!".format(vector.ndim)
    ...
    one_hot = numpy.zeros(shape=(len(vector), n_classes))
...
def safe_zip(*args):
    ...
    *args
    ...
    for i, arg in enumerate(args[1:]):
        if len(arg) != base:
            raise ValueError("Argument[0] has length {!s} but argument {!s} has "
                             "length {!s}".format(base, i+1, len(arg)))
    return zip(*args)
